...
namespace NEA
{
    public partial class gameForm : Form
    {
                    ...
                    using (NpgsqlDataReader reader = command.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            ...
                            switch (gameState)
                            {
                                ...
                                case "ended":
                                    this.Hide();
                                    online_podium onlinePodiumForm = new online_podium();
                                    onlinePodiumForm.ShowDialog();
                                    this.Close();
                                    break;
                            }
        ...
        {
            // Change every player's game_state to "end"
            using (NpgsqlConnection connection = new NpgsqlConnection(connectionString))
            {
                connection.Open();
                NpgsqlCommand command = new NpgsqlCommand("UPDATE lobby1 SET game_state = 'end'", connection);
                command.ExecuteNonQuery();
                connection.Close();
            }

            // Close the current form and open the online_podium form
            this.Hide();
            online_podium onlinePodiumForm = new online_podium();
            onlinePodiumForm.ShowDialog();
            this.Close();
